...
- Cognitive Atlas distinguishes cognitive concepts from experimental tasks. Linked memory, attention, visual-perception, and contrast-detection records are marked `Unreviewed`, so PMM mappings are provisional.
...
The next expansion is thinking and reasoning. It must separate reasoning constructs, task families, observable answers, accuracy and latency, strategy reports, psychometric ability, and candidate process models. The complete queue and promotion gate are documented in [`docs/coverage-roadmap.md`](coverage-roadmap.md).
